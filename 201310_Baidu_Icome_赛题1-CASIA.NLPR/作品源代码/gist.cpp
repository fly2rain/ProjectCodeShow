    ...
    for(fn = 0; fn < nfilters; fn++)
    {
        ...
        for(j = 0; j < height; j++)
        {
            for(i = 0; i < width; i++)
            {
                ...
                G0->data[j*G0->stride+i] = exp(-10.0f*param[fn][0]*(*fr_ptr/height/param[fn][1]-1)*(*fr_ptr/width/param[fn][1]-1)-2.0f*param[fn][2]*CV_PI*tmp*tmp);
                fr_ptr++;
            }
        }
...
float *color_gist_scaletab_gabor(color_image_t *src, int w, int n_scale, const int *n_orientation, image_list_t *G)
{
    int i;
...
float *cvGist(cv::Mat &src, int nblocks, int n_scale, int *orientations_per_scale, int &descsize)
{
    ...
    image_t *im=image_new(src.cols,src.rows);
	
    int ind=0;
    for(int i=0;i<src.rows;i++){
        for(int j=0;j<src.cols;j++){

			im->data[ind]=src.at<float>(i,j);
            ind++;
        }
    }

    float *desc=bw_gist_scaletab(im,nblocks,n_scale,orientations_per_scale);
    image_delete(im);
